...
export class DepsAdapter implements IToolAdapter {
  readonly name = 'deps' as const
  readonly metadata = TOOL_METADATA.deps
  ...
  async initialize(): Promise<void> {
    try {
      ...
      const module = await import('@ldesign/deps')
      this.deps = module
      this.status = 'active'
    } catch (error) {
      this.status = 'error'
  ...
  async dispose(): Promise<void> { this.deps = null; this.status = 'inactive' }
