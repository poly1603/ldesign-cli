...
export class MonitorAdapter implements IToolAdapter {
  readonly name = 'monitor' as const
  readonly metadata = TOOL_METADATA.monitor
  ...
  async initialize(): Promise<void> {
    try {
      ...
      const module = await import('@ldesign/monitor')
      this.monitor = module
      this.status = 'active'
    } catch (error) {
      this.status = 'error'
  ...
  async dispose(): Promise<void> { this.monitor = null; this.status = 'inactive' }
