...
export class SecurityAdapter implements IToolAdapter {
  readonly name = 'security' as const
  readonly metadata = TOOL_METADATA.security
  ...
  async initialize(): Promise<void> {
    try {
      ...
      const module = await import('@ldesign/security')
  ...
  async dispose(): Promise<void> { this.security = null; this.status = 'inactive' }
