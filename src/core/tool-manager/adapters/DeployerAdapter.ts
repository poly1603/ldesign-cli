...
export class DeployerAdapter implements IToolAdapter {
  readonly name = 'deployer' as const
  readonly metadata = TOOL_METADATA.deployer
  ...
  async initialize(): Promise<void> {
    try {
      logger.info('[DeployerAdapter] 初始化...')
      this.status = 'initializing'
      const module = await import('@ldesign/deployer')
      this.deployer = module
      this.status = 'active'
    } catch (error) {
      this.status = 'error'
      throw error
  ...
  async dispose(): Promise<void> { this.deployer = null; this.status = 'inactive' }
