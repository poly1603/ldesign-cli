...
// 请求缓存（全局单例）
const requestCache = new LRUCache<string, { data: any; timestamp: number }>(200)

/**
 * 生成缓存键
 */
function getCacheKey(url: string, options: RequestOptions): string {
  return `${options.method || 'GET'}:${url}:${JSON.stringify(options.body || options.params || {})}`
}

/**
 * 检查是否可以使用缓存
 */
function canUseCache(method: string = 'GET'): boolean {
