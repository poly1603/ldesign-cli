...
import { compareVersions, type VersionStatus } from './versionCompare'
 ...
 * 获取健康度描述
 */
export function getHealthDescription(score: HealthScore): string {
  switch (score.grade) {
    case 'A':
      return '您的依赖管理非常出色！所有依赖都保持在良好状态。'
    case 'B':
      return '依赖状况良好，少量依赖需要更新。'
    case 'C':
      return '有一些依赖需要更新，建议尽快处理。'
    case 'D':
      return '较多依赖落后，需要制定更新计划。'
    case 'F':
      return '依赖状况堪忧，建议立即采取行动更新依赖。'
    default:
      return ''
  }
}
