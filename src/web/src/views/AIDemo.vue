...
// LocalStorage 键名
const STORAGE_KEY = 'ldesign_cli_ai_conversations'
const ACTIVE_KEY = 'ldesign_cli_ai_active_conversation'
...
const messages = computed({
  get: (): Message[] => {
    ...
    return currentConversation.value.messages.map(m => ({
      role: m.role,
      content: m.content,
...
const saveConversations = () => {
  try {
    ...
    if (currentConversationId.value) {
      localStorage.setItem(ACTIVE_KEY, currentConversationId.value)
    }
  } catch (error) {
...
const loadConversations = () => {
  try {
    ...
    if (activeId && conversations.value.find(c => c.id === activeId)) {
      currentConversationId.value = activeId
...
const addMessage = (role: 'user' | 'assistant', content: string) => {
  ...
  if (role === 'user' && currentConversation.value.messages.filter(m => m.role === 'user').length === 1) {
    ...
    currentConversation.value.title = title.length > 30 ? title.substring(0, 30) + '...' : title
  }
...
const clearChat = () => {
  ...
  if (currentConversation.value.messages.length > 0 && confirm('确定清空当前对话？')) {
...
.messages-wrapper {
  ...
  overflow-x: hidden;
  padding: 24px;
  padding-bottom: 200px; // 为固定输入区域预留空间（适应多行输入框）
...
@media (max-width: 768px) {
  ...
  .messages-wrapper {
    padding-bottom: 220px; // 移动端输入区域可能更高
  }
}
</style>
