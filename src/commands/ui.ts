...
import { logger, getAccessUrls } from '@ldesign/shared/utils.js'
import { createExpressServer, getConfigManager } from '@ldesign/server'
import { DEFAULT_PORT } from '@ldesign/shared/constants.js'
...
export async function uiCommand(options: UICommandOptions = {}): Promise<void> {
  ...
  try {
    ...
    if (availablePort !== preferredPort) {
      uiLogger.warn(`端口 ${preferredPort} 已被占用，使用端�?${availablePort}`)
    }
    ...
    console.log('  ╭────────────────────────────────────�?)
    console.log('  �?                                   �?)
    console.log('  �?  🎨 LDesign UI 已启�?            �?)
    console.log('  �?                                   �?)
    console.log('  ╰────────────────────────────────────�?)
    console.log('')
    uiLogger.success('LDesign UI 管理界面已启�?)
    console.log('')
    ...
    console.log('')
    uiLogger.info('�?Ctrl+C 退�?)
    console.log('')

    // 自动打开浏览�?- 如果有网络IP,优先使用网络IP
    if (shouldOpen) {
...
/**
 * UI 命令处理�?(CommandHandler 实现)
 */
export const uiCommandHandler: CommandHandler = {
  name: 'ui',
  description: '打开可视化管理界�?,

  setup(cli: CAC) {
    cli
      .command('ui', '打开可视化管理界�?)
      .option('-p, --port <port>', '指定端口�?, { type: [Number] })
      .option('-H, --host <host>', '指定主机地址')
      .option('--no-open', '不自动打开浏览�?)
